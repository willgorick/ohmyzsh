...
function update_current_git_vars() {
    ...
    local gitstatus="$__GIT_PROMPT_DIR/scripts/gitstatus.py"
...
git_super_status() {
    ...
    if [ -n "$__CURRENT_GIT_STATUS" ]; then
        ...
        if [ "$GIT_DELETED" -ne "0" ]; then
            STATUS="$STATUS$ZSH_THEME_GIT_PROMPT_SEPARATOR$ZSH_THEME_GIT_PROMPT_DELETED$GIT_DELETED%{${reset_color}%}"
        fi
...
PROMPT="%(?: : ) %{$fg_bold[cyan]%}%~%{$reset_color%} %{$fg_bold[green]%}➜ %{$reset_color%}"
